...
def render_chat():
    ...
    if st.session_state['selected_chat_id'] is not None and st.session_state['LLM_agent'] is not None:
        st.session_state['messages'] = db_utils.load_chat_history(st.session_state['selected_chat_id'], DB_NAME, DB_USER, DB_HOST, DB_PORT, DB_PASSWORD)
        ...
        if prompt := st.chat_input("Сообщение"):
            ...
            # Сохраняем в базу
            db_utils.save_message_to_db(st.session_state['selected_chat_id'], "user", prompt, DB_NAME, DB_USER, DB_HOST, DB_PORT, DB_PASSWORD)
            db_utils.save_message_to_db(st.session_state['selected_chat_id'], "assistant", ai_answer, DB_NAME, DB_USER, DB_HOST, DB_PORT, DB_PASSWORD)

            chat_name = db_utils.get_chat_name(st.session_state['selected_chat_id'], DB_NAME, DB_USER, DB_HOST, DB_PORT, DB_PASSWORD)
            
            if chat_name == "Новый чат":
                first_message = st.session_state['messages'][0]['content']
                
                title_messages = [
                    {"role": "system", "content": "Пожалуйста, сгенерируй тему разговора на основе сообщения пользователя. Тема должна быть краткой, четкой и отражать основную тему или идею сообщения. Она должно быть простой и понятной, без использования кавычек, цифр и спецсимволов. Тема должна состоять из русских слов. Не используй вводные слова, только тема разговора."},
                ...
                db_utils.update_chat_name(st.session_state['selected_chat_id'], chat_name, DB_NAME, DB_USER, DB_HOST, DB_PORT, DB_PASSWORD)
                st.rerun()
    else:
